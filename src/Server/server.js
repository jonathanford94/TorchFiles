  ...
  db.serialize(() => {
    db.parallelize(() => {
      ...
      db.get("SELECT * FROM Country WHERE name = $name", {$name: countryName}, (err, row) => {
        ...
        if (row) {
          country.gdp = parseFloat(row.gdp).toFixed(2);
...
app.get('/country', (req, res, next) => {
  ...
  let dbQuery = "SELECT name, ROUND(gdp, 2) AS gdp, population FROM Country;";
        ...
        if (getMedalsQuery) {
          db.get(getMedalsQuery, (err, medalRow) => {
            if (err) {
              ...
              let lowerCaseRow = lowerCaseObjectKeys(medalRow);
